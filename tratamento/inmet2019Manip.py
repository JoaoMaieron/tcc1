...
def inmetD(directory,uf,year):
    ...
    for file in csv_files:
        ...
        for d in df['Data'].unique():
            ...
            if dfDay['VENTO, RAJADA MAXIMA (m/s)'].isna().all():
                new['vento_max'].append(0)
            else:
                l = []
                for t in dfDay['VENTO, RAJADA MAXIMA (m/s)']:
                    if type(t) == str:
                        l.append(float(t.replace(',','.')))
                    elif not math.isnan(t):
                        l.append(t)
                new['vento_max'].append(np.max(l))
...
# im.removeLines('../dados_inmet/2019/')
